  ...
  const handleSubmit = async (values: SelectedRowIdProp) => {
    ...
    try {
      const response = await axiosCall<ApiResponse>({
        ENDPOINT: 'vendor/delete/payment-mode',
        METHOD: 'POST',
        PAYLOAD: payload,
      });
      deleteModalToggle();
      ...
      setReload(!reload);
      if (singleVendorData) {
        const nextModes = (singleVendorData.payment_modes || []).filter((m: any) => {
          const modeStr = typeof m === 'string' ? m : (m?.mode ?? '');
          return modeStr !== values.mode;
        });
        dispatch(
          setSingleVendorData({
    ...
    >
      {rowId ? (
        <div className="modal-toggle-wrapper">
          ...
          <H4 className="text-center pb-2">Delete this Payment Mode?</H4>
          <Btn
            color="danger"
