  ...
  const handleSubmit = async (values: SelectedRowIdProp) => {
    ...
    try {
      const response = await axiosCall<ApiResponse>({
        ENDPOINT: 'vendor/delete/material',
        METHOD: 'POST',
        PAYLOAD: payload,
      });
      deleteModalToggle();
      ...
      setReload(!reload);
      if (singleVendorData) {
        const nextMaterials = (singleVendorData.materials || []).filter(
          (m: any) => String(m.id) !== String(values.material_id)
        );
        dispatch(
          setSingleVendorData({
    ...
    >
      {rowId ? (
        <div className="modal-toggle-wrapper">
          ...
          <H4 className="text-center pb-2">Delete this Material?</H4>
          <Btn
            color="danger"
